...
async def process_add_procedure(message: Message, state: FSMContext):
    ...
    if up := add_user_procedure(session, data['user'].id, data['proc_id'], int(message.text)):
        cur_proc = get_cur_month_procedure(session, data['user'].id)
        money_month = 0
        money_day = 0
        money_curr = 0
        for proc, count, created_at, user_procedure_id in cur_proc:
            if proc.rate == 0 or count == 0:
                continue
            if up.id == user_procedure_id:
        ...
        await message.answer(f"<em>Успешно</em>\n\n"
                             f"Заработано за текущую операцию: <b>{round(money_curr, 1)} руб.</b>\n"
                             f"Заработано за текущий день: <b>{round(money_day, 1)} руб.</b>\n"
